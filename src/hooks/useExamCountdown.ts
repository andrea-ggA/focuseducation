...
import { getStartOfLocalDay, getStartOfLocalWeek } from "@/lib/datetime";
...
export function useExamCountdown() {
    ...
    try {
      const [profileRes, focusRes] = await Promise.allSettled([
        ...
        supabase
          ...
          .eq("user_id", user.id)
          .eq("completed", true)
          .gte("started_at", getStartOfLocalWeek().toISOString()),
      ]);
      ...
      if (profileRes.status === "fulfilled" && profileRes.value.data) {
        ...
        if (info.exam_date) {
          ...
          const examDay = getStartOfLocalDay(new Date(`${info.exam_date}T00:00:00`));
  ...
  const saveExam = useCallback(async (date: string, subject: string) => {
    ...
    await supabase
      .from("profiles")
      .update({ exam_date: date, exam_subject: subject })
      .eq("user_id", user.id);
