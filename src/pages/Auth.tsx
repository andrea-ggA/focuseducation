...
const Auth = () => {
  ...
  // ─── Google OAuth ─────────────────────────────────────────────────────────
  // Bypasses the Lovable cloud-auth intermediary and calls Supabase directly.
  // Previous flow: Browser → Lovable servers → Google → Lovable → Browser
  // Fixed flow:    Browser → Supabase → Google → Browser  (one less hop, faster)
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          // After Google auth, Supabase redirects here; the JS client reads the
          // hash tokens automatically via onAuthStateChange → SIGNED_IN.
          redirectTo: `${window.location.origin}/dashboard`,
          queryParams: {
            // Request offline_access so the token can be refreshed silently
            access_type: "offline",
            // Force account selection every time (avoids silent wrong-account login)
      ...
      // If we reach here without redirect, something odd happened
    } catch (err: any) {
