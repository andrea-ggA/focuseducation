...
const CONCURRENCY     = 4;
const ITEMS_PER_CHUNK = 15;  // domande o flashcard per chunk (era 30)
  ...
  while (start < text.length) {
    ...
    if (end < text.length) {
      ...
      if (lastDouble > maxChars * 0.55) {
        ...
        if (lastSentence > 0) { end = start + lastSentence; }
        else {
...
async function parallelLimit<T>(tasks: (() => Promise<T>)[], limit: number): Promise<T[]> {
  ...
  async function worker() {
    while (nextIndex < tasks.length) { const idx = nextIndex++; results[idx] = await tasks[idx](); }
  }
...
async function updateJob(
  ...
  if (Object.keys(fallback).length === 0 || Object.keys(fallback).length === Object.keys(updates).length) {
    console.warn("generation_jobs update failed:", error.message); return;
  }
...
async function extractTopicOutline(apiKey: string, text: string, language: string): Promise<string[]> {
  ...
  try {
    ...
    ], FAST_MODEL, 2, 1500); // FAST_MODEL ovunque — nessun gemini-2.5-flash hardcoded
...
function consolidateTopics<T extends Record<string, any>>(items: T[], outline: string[] = [], max = 18): T[] {
  ...
  if (outline.length > 0) {
    items = items.map((item: T) => {
      ...
      for (const ot of outline) { const s = topicOverlap(t, ot); if (s > bestScore) { bestScore = s; best = ot; } }
      if (bestScore < 0.15) {
        for (const ot of outline) {
          if (normCmp(t).includes(normCmp(ot)) || normCmp(ot).includes(normCmp(t))) { best = ot; bestScore = 0.5; break; }
        }
      }
  ...
  for (const [topic] of sorted.slice(max)) {
    ...
    for (const [kt] of kept) { const s=topicOverlap(topic,kt); if(s>bestS){bestS=s;best=kt;} }
  ...
  try {
    ...
    if (hasImages) {
      ...
      const aiData = await callAI(GEMINI_API_KEY,
        [{ role:"system", content:`Professional academic educator. ${lang}` }, { role:"user", content:parts }],
        FAST_MODEL, 3, 16000);
    ...
    const [docLanguage, topicOutline] = await Promise.all([
      heuristicLang
        ...
        : callAI(GEMINI_API_KEY,
            ...
            FAST_MODEL, 2, 15,
          ).then(d => { const det = d.choices?.[0]?.message?.content?.trim().toLowerCase() || ""; return det && det.length < 25 ? det : "italiano"; })
          .catch(() => "italiano"),
    ...
    const chunkTasks = chunks.map((chunk, idx) => async () => {
      const isFirst = idx === 0, chunkNum = idx + 1, isQ = type !== "flashcards";
