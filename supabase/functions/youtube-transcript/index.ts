...
serve(async (req) => {
  ...
  try {
    // FIX #4: Added authentication — previously anyone could call this endpoint
    ...
    // FIX #9: Added rate limiting (previously missing — could abuse Gemini API)
    ...
    if (!url) {
      return new Response(JSON.stringify({ error: "URL YouTube richiesto." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!videoId) {
      return new Response(JSON.stringify({ error: "URL YouTube non valido." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    try {
      ...
      if (captionMessage === "NO_CAPTIONS" || captionMessage.includes("sottotitoli")) {
        ...
        if (!geminiKey) {
          return new Response(JSON.stringify({ error: "Servizio di analisi video AI non disponibile." }), {
            status: 422,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
...
Fornisci SOLO la trascrizione/contenuto testuale, senza commenti o prefissi.`,
        ...
        if (!geminiRes.ok) {
          ...
          return new Response(JSON.stringify({ error: "Impossibile analizzare il video. Riprova più tardi." }), {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (aiContent.length < 100) {
          return new Response(JSON.stringify({ error: "L'analisi del video ha prodotto contenuto insufficiente." }), {
            status: 422,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
